...
def main(argv: list[str] | None = None) -> int:
    ...
    _write_grid_json(grid_output, cells, username, config)
    _write_snake_svg(svg_output, cells, config)
...
def _write_grid_json(path: Path, cells, username: str, config: AppConfig) -> None:
    payload = {
...
def _write_snake_svg(path: Path, cells, config: AppConfig) -> None:
    svg = render_animated_snake(
        cells,
        weeks=config.grid.weeks,
